...
{
    public partial class frmManageIssueList : Form
    {
        public frmManageIssueList()
        {
            InitializeComponent();
        }
        ...
        public void autoRefresh()
        {
            ...
            try
            {
                DataTable dt = bookList.GetData("SELECT ID, Library_ID, User, Title, Accession, Issue_Date, Expected_Return, Date_Return, Status FROM tblbookassign ORDER BY Issue_Date ASC ");
        ...
        private void frmManageIssueList_Load(object sender, EventArgs e)
        {
            autoRefresh();
        }
        ...
        private void txtSearch_TextChanged(object sender, EventArgs e)
        {
            try
            {
                DataTable dt = bookList.GetData("SELECT ID, Library_ID, User, Title, Accession, Issue_Date, Expected_Return, Date_Return, Status FROM tblbookassign WHERE " +
                    "('Library_ID' LIKE '%" + txtSearch.Text + "%' OR Title LIKE '%" + txtSearch.Text + "%' OR User LIKE '%" + txtSearch.Text + "%' OR Status LIKE '%" + txtSearch.Text + "%') ORDER BY Issue_Date ASC ");
                dataGridView1.DataSource = dt;
                dataGridView1.Columns[1].Visible = false;
                dataGridView1.Columns[4].Visible = false;
                dataGridView1.Columns[6].Visible = false;
                            ...
                            if (checkbox.Value != null && (bool)checkbox.Value == true)
                            {
                                ...
                                if (bookList.rowAffected > 0)
                                {
                                    ...
                                    if (bookList.rowAffected > 0)
                                    {
                                        ...
                                        + DateTime.Now.ToLongTimeString() + "','Issue & Return List','Bookrequest ID " + ID + " status is changed to returned')");
                                    }
                                }
                            }
                        }
                        ...
                        for (int i = 0; i < dataGridView1.Rows.Count; i++)
                        {
                            ...
                            if (checkbox.Value != null && (bool)checkbox.Value == true)
                            {
                                ...
                                if (bookList.rowAffected > 0)
                                {
                                    ...
                                    bookList.executeSQL("INSERT INTO logs (date, time, module, action) VALUES('" + DateTime.Now.ToString("yyyy-MM-dd ") + "','"
                                        + DateTime.Now.ToLongTimeString() + "','Issue & Return List','Bookrequest ID " + ID + " deleted in return list')");
                                }
                            }
                        }
        ...
        private void cmbType_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                DataTable dt = bookList.GetData("SELECT ID, Library_ID, User, Title, Accession, Issue_Date, Expected_Return, Date_Return, Status FROM tblbookassign WHERE Status = 'RETURNED' ORDER BY Issue_Date ASC ");
                dataGridView1.DataSource = dt;
            }
            else
            {
                DataTable dt = bookList.GetData("SELECT ID, Library_ID, User, Title, Accession, Issue_Date, Expected_Return, Date_Return, Status FROM tblbookassign WHERE Status = 'OUT' ORDER BY Issue_Date ASC ");
                dataGridView1.DataSource = dt;
            }           
            dataGridView1.Columns[1].Visible = false;
            dataGridView1.Columns[4].Visible = false;
