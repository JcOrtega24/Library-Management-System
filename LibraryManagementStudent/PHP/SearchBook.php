...
<body>
    ...
    <?php
        ...
        if (isset($_POST['chk_id'])) {
            ...
            if (empty($errors)) {
                ...
                $insertedAccessions = array();

                // Prepare the insert statement
                $insertSql = "INSERT INTO tblbookrequest (Accession, requestedBy, dateRequest, statusRequest) VALUES (?, ?, ?, ?)";
                if ($insertStmt = mysqli_prepare($link, $insertSql)) {
                    foreach ($arr as $id) {
                        // Check if the Accession number is already in the tblbookrequest table
                        $sql = "SELECT * FROM tblbookrequest WHERE Accession = ? AND requestedBy = ?";
                        if ($stmt = mysqli_prepare($link, $sql)) {
                            mysqli_stmt_bind_param($stmt, "ss", $id, $user);
                            if (mysqli_stmt_execute($stmt)) {
                                $result = mysqli_stmt_get_result($stmt);
                                if (mysqli_num_rows($result) > 0) {
                                    // If the Accession number already exists, skip the current iteration
                                    $errors[] = "The selected book with Accession " . $id . " has already been requested by you.";
                                    continue;
                                }
                            }
                        }
                        ...
                        if ($stmt = mysqli_prepare($link, $sql)) {
                            ...
                            if (mysqli_stmt_execute($stmt)) {
                                ...
                                if (mysqli_num_rows($result) > 0) {
                                    ...
                                    $sql = "SELECT COUNT(*) AS num_reservations FROM tblbookrequest WHERE requestedBy = ?";
                                    if ($stmt2 = mysqli_prepare($link, $sql)) {
                                        mysqli_stmt_bind_param($stmt2, "s", $user);
                                        if (mysqli_stmt_execute($stmt2)) {
                                            $result2 = mysqli_stmt_get_result($stmt2);
                                            $row = mysqli_fetch_assoc($result2);
                                            if ($row['num_reservations'] >= 3) {
                                                $errors[] = "You have already reserved 3 books. You cannot reserve more than 3 books.";
                                                continue;
                                            }
                                        }
                                    }
                                    // Insert the book request into the tblbookrequest table
                                    mysqli_stmt_bind_param($insertStmt, "ssss", $id, $user, $date, $status);
                                    mysqli_stmt_execute($insertStmt);
                                    $insertedAccessions[] = $id;
                                }
                            } else {
                                $errors[] = "Error On Select Statement!";
                            }
                        }
                    }
                                ...
                                function build_table($result)
                                {
                                    if(mysqli_num_rows($result) > 0)
                                    {
                                        ...
                                        while($row = mysqli_fetch_array($result))
                                        {
                                            ...
                                            $ID = $row['Id'];
                                            echo "<tr>";
                                                if($row['Type'] != "Books" || $row['Status'] != "Available")
                                                {
                                                    ?>
                                                    <td>
                                                        <span class="custom-checkbox">
                                                            <input type="checkbox" id="checkbox2" name="chk_id[]" value="<?php echo $row['Accession'];?>" hidden disabled>
                                                    ...
                                                    <div class="modal-content">
                                                        ...
                                                        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method = "POST" onsubmit="return validateForm()">   
                                                            ...
                                                            <div class="modal-footer">
                                                                ...
                                                                <input type="submit" class="btn btn-danger" id="submit" name="submit" style='font-weight: bold; background-color: #218838; color:#FFFFFF' value="Request">
                                                            </div>
                                                        </form>
                                                    </div>
                                                </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            var checkboxes = $("input[name='chk_id[]']:not(:disabled)");
            var maxCheckboxes = 3;
            ...
            $("#chk_all").change(function() {
                ...
                if (this.checked) {
                    checkboxes.filter(":not(:checked)").slice(0, maxCheckboxes).prop("checked", true);
                }
            });
        });
    </script>
